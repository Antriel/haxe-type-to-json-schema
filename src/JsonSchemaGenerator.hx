...
class JsonSchemaGenerator {
    ...
    static inline var REF_KEY:String = "$ref";
    #else
    static inline var SCHEMA_KEY:String = "@$__hx__$schema";
    static inline var REF_KEY:String = "@$__hx__$ref";
    #end
    static inline var FIELD_NAME_TYPE:String = "type";
    static inline var ENUM_META:String = ":enum";
    ...
    public static function getAbstractEnumValues(typePath:Expr):Expr {
        ...
        switch (type) {
            case TAbstract(_.get() => ab, _) if (ab.meta.has(ENUM_META)):
                ...
                for (field in ab.impl.get().statics.get()) {
                    if (field.meta.has(ENUM_META) && field.meta.has(":impl")) {
    ...
    public static function genSchema(type:Type, typeName:String, pos:Position, structInfo:Null<StructInfo>, refs:DynamicAccess<Expr>, order:Int,
        ...
        switch (type) {
            case TType(_.get() => dt, params):
                return switch [dt, params] {
                    ...
                    default:
                        ...
                        return SchemaUtils.makeObjectDecl([{field: REF_KEY, expr: macro '#/definitions/${dt.name}'}], structInfo, order, pos);
                }
            ...
            case TAbstract(_.get() => ab, params):
                switch [ab, params] {
                    case [{pack: [], name: "Int"}, []]:
                        var fields:Array<ObjectDeclField> = [{field: FIELD_NAME_TYPE, expr: macro "integer"}];
                        if (extendCB != null) {
                            extendCB(fields, typeName, pos, refs);
                        }
                        return SchemaUtils.makeObjectDecl(fields, structInfo, order, pos);
                    case [{pack: [], name: "Float"}, []]:
                        return SchemaUtils.makeObjectDecl([{field: FIELD_NAME_TYPE, expr: macro "number"}], structInfo, order, pos);
                    case [{pack: [], name: "Bool"}, []]:
                        return SchemaUtils.makeObjectDecl([{field: FIELD_NAME_TYPE, expr: macro "boolean"}], structInfo, order, pos);
                    case [{pack: [], name: "Any"}, []]:
                        return SchemaUtils.makeObjectDecl([{field: FIELD_NAME_TYPE, expr: macro "object"}], structInfo, order, pos);
                    case [{pack: [], name: "Null"}, [realT]]:
                        return genSchema(realT, typeName, pos, structInfo, refs, order, extendCB);
                    case [{type: t}, []]:
                        return genSchema(t, typeName, pos, null, refs, order, extendCB);
                    default:
                        if (ab.meta.has(ENUM_META)) {
                            if (structInfo == null) {
                                structInfo = SchemaUtils.makeStructInfo(ab.name, ab.doc);
                            }
                            var pack:Array<String> = ab.module.split(".");
                            if (pack[pack.length - 1] != ab.name) {
                                pack.push(ab.name);
                            }
